...
###CREATING SPECTROGRAMS FOR ALL THE AUDIO SAMPLES
print("Creating Spectrograms..")
for row in range(len(A_dir)):
    y,sr = librosa.load(A_dir[row],duration=3)
    hann=np.hamming(len(y))
    y=y*hann
    ps = librosa.feature.melspectrogram(y=y,sr=sr)
    D1.append((ps,int(label_A[row])))
len(D1)

for row in range(len(B_dir)):
    y,sr = librosa.load(B_dir[row],duration=3)
    hann=np.hamming(len(y))
    y=y*hann
    ps = librosa.feature.melspectrogram(y=y,sr=sr)
    D2.append((ps,int(label_B[row])))
len(D2)

for row in range(len(C_dir)):
    y,sr = librosa.load(C_dir[row],duration=3)
    hann=np.hamming(len(y))
    y=y*hann
    ps = librosa.feature.melspectrogram(y=y,sr=sr)
    D3.append((ps,int(label_C[row])))
len(D3)

for row in range(len(D_dir)):
    y,sr = librosa.load(D_dir[row],duration=3)
    hann=np.hamming(len(y))
    y=y*hann
    ps = librosa.feature.melspectrogram(y=y,sr=sr)
    D4.append((ps,int(label_D[row])))
len(D4)
...
for m in range(4):
    ...
    train_x = np.array([x.reshape( (128, 130, 1) ) for x in train_x])
    train_y=train["bc"].values
    test_x = np.array([x.reshape( (128, 130, 1) ) for x in test_x])
    ...
    model=tf.keras.models.Sequential([
        tf.keras.layers.Conv2D(64,(3,3),input_shape=[128,130,1],activation="relu"),
